<template>
  <RouterLink :to="`/items/${item.id}`" class="ecard">
    <div class="img">
      ...
      <span class="cat" :class="`tone-${CATEGORY[item.category]?.tone ?? 'gray'}`">{{ CATEGORY[item.category]?.label ?? '기타' }}</span>
      <button class="heart" :class="{ on: liked, pop }" :aria-pressed="liked" :aria-label="liked ? '좋아요 취소' : '좋아요'" @click.prevent.stop="toggle">
        <Heart :size="19" :stroke-width="2.2" :fill="liked ? 'currentColor' : 'none'" />
      </button>
    </div>
    <div class="body">
      <b>{{ item.title }}</b>
...
.cat { position: absolute; left: 10px; top: 10px; backdrop-filter: blur(6px); box-shadow: 0 1px 4px rgba(0, 0, 0, .08); }
.heart { position: absolute; right: 10px; top: 10px; width: 36px; height: 36px; padding: 0; border-radius: 50%; background: rgba(255, 255, 255, .92); color: var(--ink); display: grid; place-items: center; box-shadow: 0 2px 8px rgba(0, 0, 0, .12); }
