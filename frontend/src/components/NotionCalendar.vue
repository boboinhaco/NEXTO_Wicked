<template>
  <div class="ncal">
    <div class="bar">
      ...
      <div class="ctrl">
        <button class="text" aria-label="이전 달" @click="shift(-1)">‹</button>
        <button class="text" @click="emit('update:month', startOfMonth(new Date()))">오늘</button>
        <button class="text" aria-label="다음 달" @click="shift(1)">›</button>
      </div>
    </div>
    <div class="scroll">
      ...
      <div v-for="(w, wi) in weeks" :key="wi" class="week" :style="{ gridTemplateRows: `28px repeat(${Math.max(w.lanes, 1)}, 24px) ${w.more.some(Boolean) ? '20px' : ''} 1fr` }">
        <div v-for="(d, di) in w.days" :key="d.key" class="day" :class="{ out: !d.inMonth, weekend: di === 0 || di === 6 }" :style="{ gridColumn: di + 1 }">
          <span class="num" :class="{ today: d.today }">{{ d.date.getDate() === 1 ? `${d.date.getMonth() + 1}월 1` : d.date.getDate() }}</span>
        </div>
...
const props = defineProps({ events: { type: Array, default: () => [] }, month: { type: Date, required: true }, maxLanes: { type: Number, default: 3 }, linkable: { type: Boolean, default: true } })
...
.bar strong { font-family: var(--serif); font-size: 15px; }
.ctrl { display: flex; gap: 2px; }
.ctrl button { font-size: 14px; }
.scroll { overflow-x: auto; border-top: 1px solid var(--line); }
.head, .week { display: grid; grid-template-columns: repeat(7, minmax(92px, 1fr)); min-width: 644px; }
...
.num.today { background: #eb5757; color: #fff; font-weight: 600; }
.ev { position: relative; z-index: 1; margin: 1px 4px; padding: 0 8px; border-radius: 4px; font-family: var(--serif); font-size: 12.5px; line-height: 24px;
